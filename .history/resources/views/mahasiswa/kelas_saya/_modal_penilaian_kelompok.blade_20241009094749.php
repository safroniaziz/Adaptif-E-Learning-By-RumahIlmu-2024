            ...
            <div class="modal-body">
                ...
                <form id="formPenilaian" action="{{ route('mahasiswa.kelas_saya.simpan_penilaian', [$materi->topikPembahasanKelas->kelas->id, $materi->id, $tugas->id]) }}" method="POST">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" id="submitBtn">Simpan Penilaian</button> <!-- Ganti type jadi button -->
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
       $('#formPenilaian').on('submit', function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    // Jika ada error validasi, tampilkan error di SweetAlert
                    if (xhr.status === 422) { // Status 422 adalah error validasi Laravel
                        var errors = xhr.responseJSON.errors;
                        var errorMessages = '';

                        $.each(errors, function (key, value) {
                            errorMessages += value[0] + '<br>'; // Gabungkan semua pesan error
                        });

                        Swal.fire({
                            title: 'Error!',
                            html: errorMessages, // Menampilkan semua pesan error
                            icon: 'error',
                            confirmButtonText: 'OK'
                        });
                    }

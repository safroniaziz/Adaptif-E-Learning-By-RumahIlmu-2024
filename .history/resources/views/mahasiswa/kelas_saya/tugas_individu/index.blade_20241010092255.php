    ...
    @foreach ($materi->tugasIndividus as $index => $tugas)
        <div class="mb-1">
            ...
            <div class="mb-15">
                ...
                @php
                    $currentTime = \Carbon\Carbon::now(); // Menggunakan Carbon untuk waktu sekarang dengan timezone Laravel
                    $deadlineTime = \Carbon\Carbon::parse($tugas->waktu_selesai);
                    $startTime = \Carbon\Carbon::parse($tugas->waktu_mulai);

                    $statusUpload = $tugas->status_upload == 1;

                    $sudahDitambahkan = \App\Models\PengumpulanTugasIndividuDetail::join('pengumpulan_tugas_individus','pengumpulan_tugas_individus.id','pengumpulan_tugas_individu_details.pengumpulan_tugas_individu_id')
                                    ...
                                    ->where('tugas_individu_materis.id', $tugas->id)
                                    ->first();
                ...
                @elseif ($currentTime >= $startTime && $currentTime >= $deadlineTime && $isComplete)
                                ...
                                @if ($st)
                                    <div class="mt-3">
                                        <form id="form-hapus-tugas" action="{{ route('mahasiswa.kelas_saya.hapusFile_tugas_individu', [$kelas->id, $materi->id, $tugas->id]) }}" method="POST">
                                            @csrf @method('DELETE')
                                            <button typatue="button" class="btn btn-danger btn-sm btn-flat" id="btn-hapus-tugas">
                ...
                @elseif (!$isComplete)
                    ...
                    @if ($currentTime->gte($startTime) && $currentTime->lt($deadlineTime))

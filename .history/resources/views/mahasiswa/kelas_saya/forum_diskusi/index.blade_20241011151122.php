            ...
            @foreach ($diskusis as $diskusi)
                <div class="card card-flush mb-10">
                    ...
                    <div class="card-footer pt-0">
                                ...
                                @foreach ($diskusi->diskusiRespons as $respon)
                                    <div class="d-flex pt-6 align-items-center justify-content-between">
                                        ...
                                        @if($respon->canDelete && is_null($respon->nilai))
                                            <button class="btn btn-icon btn-light btn-sm btn-edit-respon" data-id="{{ $respon->id }}" data-pesan="{{ $respon->pesan }}">
                        ...
                        <!--end::Info-->
                        <!--begin::Comment form-->
                        <!-- Form untuk balasan, letakkan di luar loop -->
<form id="formBalas" action="" method="POST">
    ...
    <div class="d-flex align-items-center">
        ...
        <div class="position-relative w-100">
            ...
            <button type="button" class="btn btn-icon btn-sm btn-color-gray-500 btn-active-color-primary w-25px p-0 position-absolute top-50 end-0 translate-middle-y me-8 d-flex align-items-center btn-submit-balas">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.btn-edit-respon').forEach(button => {
        button.addEventListener('click', function() {
            var responId = this.getAttribute('data-id'); // ID dari respon yang di klik
            var pesan = this.getAttribute('data-pesan'); // Isi pesan yang akan dimasukkan ke form
            var diskusiId = this.closest('.card-footer').querySelector('.btn-submit-balas').getAttribute('data-diskusi-id'); // Ambil diskusi_id dari tombol balas

            // Cari form balasan global
            var formBalas = document.getElementById('formBalas');

            if (formBalas) {
                // Ubah action form agar sesuai dengan diskusi dan respon yang sedang diedit
                formBalas.setAttribute('action', '/kelas_saya/' + diskusiId + '/balas/' + responId);

                // Masukkan pesan ke textarea form
                var textarea = formBalas.querySelector('textarea[name="respon"]');
                textarea.value = pesan;

                // Tambahkan hidden input untuk respon ID agar tahu respon mana yang sedang diedit
                var inputId = formBalas.querySelector('input[name="respon_id"]');
                if (!inputId) {
                    // Jika hidden input untuk respon_id belum ada, buat baru
                    inputId = document.createElement('input');
                    inputId.setAttribute('type', 'hidden');
                    inputId.setAttribute('name', 'respon_id');
                    inputId.setAttribute('value', responId);
                    formBalas.appendChild(inputId);
                } else {
                    // Jika sudah ada, update nilainya
                    inputId.value = responId;
                }

                // Fokus ke textarea atau scroll ke form balasan
                textarea.focus();
                formBalas.scrollIntoView({ behavior: 'smooth' });
            } else {
                console.error('Form balasan tidak ditemukan');
            }
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.btn-submit-balas').forEach(button => {
            button.addEventListener('click', function() {
                var diskusiId = this.getAttribute('data-diskusi-id'); // Ambil ID diskusi yang relevan

                // Cari form balasan global
                var formBalas = document.getElementById('formBalas');
                formBalas.setAttribute('action', '/kelas_saya/' + diskusiId + '/balas');

                // Submit form dengan fetch API
                submitReplyForm(formBalas);
            });
        });
        ...
        .then(data => {
            if (data.errors) {
                ...
                Swal.fire('Error!', data.message || 'Terjadi kesalahan saat mengirim balasan.', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            Swal.fire('Error!', error.message || 'Terjadi kesalahan saat mengirim balasan.', 'error');
        });
    }

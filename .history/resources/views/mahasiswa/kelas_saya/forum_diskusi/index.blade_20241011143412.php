...
@section('contentMateri')
    ...
    <div class="d-flex pt-6 align-items-center justify-content-between">
        ...
        @if($respon->canDelete && is_null($respon->pesan))
...
<script>
    ...
    document.querySelectorAll('.btn-edit-respon').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Dapatkan form yang sesuai dengan ID balasan
            var form = document.querySelector('#formBalas-' + responId);

            if (form) {
                // Masukkan pesan ke dalam form input
                document.querySelector('textarea[name="respon"]').value = pesan;

                // Tambahkan hidden input untuk respon ID agar tahu respon mana yang diedit
                if (!document.querySelector('input[name="respon_id"]')) {
                    ...
                    // Jika hidden input sudah ada, update nilainya
                    document.querySelector('input[name="respon_id"]').value = responId;
                }

                // Scroll ke form atau fokuskan form

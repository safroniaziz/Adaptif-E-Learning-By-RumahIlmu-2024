                                ...
                                @foreach ($diskusi->diskusiRespons as $respon)
                                    <div class="d-flex pt-6 align-items-center justify-content-between">
                                        ...
                                        @if($respon->canDelete && is_null($respon->nilai))
                                            <button class="btn btn-icon btn-light btn-sm btn-edit-respon" data-id="{{ $respon->id }}" data-pesan="{{ $respon->pesan }}" data-bs-toggle="modal" data-bs-target="#editResponModal">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="formEditRespon" method="POST">
                            @csrf
                            @method('POST')
                            <input type="hidden" name="respon_id" id="responId">
                            <div class="mb-3">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        editResponModal.addEventListener('show.bs.modal', function (event) {
            ...
            var responId = button.getAttribute('data-id'); // Ambil ID respon
            var pesan = button.getAttribute('data-pesan'); // Ambil pesan respon

            // Ganti placeholder ':respon' dengan ID respon yang benar di URL
            var actionUrl = updateResponRoute.replace(':respon', responId);

            // Isi form di dalam modal
            var modal = this; // Modal saat ini
            modal.querySelector('#responId').value = responId; // Set nilai responId
            modal.querySelector('#pesan').value = pesan; // Set pesan balasan

            // Set action form sesuai dengan respon yang diedit
            var formEdit = modal.querySelector('#formEditRespon');
            formEdit.setAttribute('action', actionUrl); // Atur URL action untuk form
        });
    });
    ...
    formEditRespon.onsubmit = function(e) {
        ...
        .then(response => {
            ...
            if (contentType && contentType.includes('application/json')) {
                return response.json(); // Jika respon JSON, parsing sebagai JSON
            } else {
                throw new Error('Respon bukan JSON. Mungkin terjadi error pada server.');
            }
        })
        .then(data => {
            if (data.success) {
        ...
        .catch(error => {
            // Menampilkan pesan error lebih rinci dari server
            Swal.fire({
                icon: 'error',
                title: 'Error!',
                text: error.message || 'Terjadi kesalahan saat memproses permintaan.'
            });
            console.error('Error:', error); // Tambahkan log ke konsol untuk detail error lebih lanjut
        });
    };

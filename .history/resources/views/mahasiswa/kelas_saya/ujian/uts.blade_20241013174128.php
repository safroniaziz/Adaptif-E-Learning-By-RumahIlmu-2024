...
function submitForm(isTimeUp = false) {
    const form = document.getElementById('kuisionerForm');
    const formData = new FormData(form);

    // Tambahkan flag untuk menandai submission karena waktu habis
    if (isTimeUp) {
...
// Modifikasi fungsi showTimeUpAlert
function showTimeUpAlert() {
    Swal.fire({
        title: 'Waktu Habis!',
        text: 'Waktu untuk mengerjakan UTS telah habis. Jawaban Anda akan disimpan.',
        ...
        if (result.isConfirmed) {
            submitForm(true);  // Panggil submitForm dengan flag isTimeUp
        }
    });
}

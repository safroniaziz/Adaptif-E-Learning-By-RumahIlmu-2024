...
function submitForm(isTimeUpSubmission = false) {
    if (isSubmitting && !isTimeUpSubmission) return; // Jangan submit lagi jika sudah dalam proses submit
...
function showTimeUpAlert() {
    if (isSubmitting) return; // Jangan tampilkan alert jika sedang proses submit

    isSubmitting = true;
    Swal.fire({
        title: 'Waktu Habis!',
        text: 'Waktu untuk mengerjakan UTS telah habis. Jawaban Anda akan disimpan secara otomatis.',
        icon: 'warning',
        allowOutsideClick: false,
        allowEscapeKey: false,
        allowEnterKey: false,
        showConfirmButton: false,
        timer: 5000,  // Alert akan tertutup setelah 5 detik

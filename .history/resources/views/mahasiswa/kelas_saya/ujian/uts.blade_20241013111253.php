...
function startCountdown(startTime, endTime) {
    const form = document.getElementById('kuisionerForm'); // Pastikan mengambil form yang benar
    ...
    function updateCountdown() {
        ...
        // Fase 3: Jika waktu ujian habis
            ...
            isTimeUp = true;
            console.log('Waktu habis. Menampilkan SweetAlert...'); // Tambahkan logging untuk debugging
...
function showTimeUpAlert(endTime) {
    const now = new Date().getTime(); // Waktu sekarang dalam milidetik
    const oneMinuteAfterEndTime = endTime + 60000; // 1 menit setelah endTime
    let timeLeft = Math.floor((oneMinuteAfterEndTime - now) / 1000); // Sisa waktu dalam detik
    ...
    if (timeLeft <= 0) {
        // Jika waktu sudah habis, langsung submit form
        submitForm();
        return;
    }

    // Tampilkan SweetAlert dengan countdown
    Swal.fire({
        title: 'Waktu Ujian Telah Berakhir!',
        html: `Silakan klik tombol "Simpan Jawaban" dalam <b>${timeLeft}</b> detik untuk menyelesaikan ujian.`,
        icon: 'warning',
        ...
        willOpen: () => {
            ...
            const timerInterval = setInterval(() => {
                timeLeft--;
                b.textContent = timeLeft; // Perbarui teks countdown

                // Jika countdown habis, submit jawaban otomatis
                if (timeLeft <= 0) {
                    clearInterval(timerInterval); // Hentikan interval
                    Swal.close(); // Tutup SweetAlert
                    submitForm(); // Submit form otomatis
                }
        ...
        if (result.isConfirmed) {
            submitForm(); // Jika tombol "Simpan Jawaban" diklik
        }
    });
}
...
function submitForm() {
    ...
    fetch(form.action, {
        ...
        Swal.fire({
            ...
            const endTime = new Date().getTime(); // Hitung ulang waktu dari server
            showTimeUpAlert(endTime); // Jika gagal, tetap tampilkan SweetAlert dengan countdown
        });
    });
}

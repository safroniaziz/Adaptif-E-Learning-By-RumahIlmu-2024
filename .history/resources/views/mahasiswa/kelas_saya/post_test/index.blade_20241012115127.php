...
@push('scripts')
    ...
    function submitForm() {
        ...
        .then(({status, body}) => {
            if (status === 200 && typeof body === 'object' && body.success) {
                Swal.fire({
                    title: 'Berhasil!',
                    text: body.message,
                    icon: 'success',
                ...
                if (typeof body === 'object') {
                    ...
                    // If the response is HTML, extract the error message
                    const parser = new DOMParser();
                    const htmlDoc = parser.parseFromString(body, 'text/html');
                    const errorElement = htmlDoc.querySelector('.error-message') || htmlDoc.querySelector('title');
                    errorMessage = errorElement ? errorElement.textContent : 'Terjadi kesalahan yang tidak diketahui.';
                }

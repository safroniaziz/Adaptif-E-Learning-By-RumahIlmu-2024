    ...
    <script>
       ...
       function submitForm() {
           ...
           fetch(form.action, {
               ...
               credentials: 'same-origin' // Ini penting untuk mengirim cookies
           })
           .then(response => {
               if (!response.ok) {
                   if (response.status === 419) { // CSRF token mismatch
                       throw new Error('Sesi Anda telah kedaluwarsa. Silakan muat ulang halaman dan coba lagi.');
                   }
           ...
           .then(data => {
               if (data.success) {
                   ...
                   Swal.fire('Error!', data.message || 'Terjadi kesalahan saat menyimpan jawaban.', 'error');
               }
           })
           .catch(error => {

        ...
        if (result.isConfirmed) {
            ...
            fetch(form.action, {
                ...
                Swal.fire(
                    ...
                    window.location.reload(); // Reload halaman jika diperlukan
                });
            }).catch(error => {
                let errorMessages = '';
                if (error.errors) {
                    // Pastikan error tidak ditambahkan berkali-kali, hanya sekali
                    const uniqueErrors = new Set(); // Gunakan Set untuk memastikan pesan unik

                    for (const key in error.errors) {
                        if (error.errors.hasOwnProperty(key)) {
                            uniqueErrors.add(error.errors[key].join('<br>')); // Tambahkan hanya yang unik
                        }
                    }
                    // Gabungkan pesan error yang unik menjadi satu

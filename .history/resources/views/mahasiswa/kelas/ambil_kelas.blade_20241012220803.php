            ...
            if (result.isConfirmed) {
                ...
                fetch(form.action, {
                    ...
                    headers: {
                        ...
                        'Accept': 'application/json' // Penting agar Laravel mengembalikan JSON pada validasi error
                    }
                }).then(response => {
                    if (!response.ok) {
                        // Jika respons tidak OK (misal: validasi error), lempar respons untuk ditangani di catch
                    ...
                    Swal.fire(
                        'Berhasil!',
                        data.message || 'Jawaban Anda berhasil disimpan.',
                        'success'
                    ).then(() => {
                        window.location.reload(); // Reload halaman atau arahkan sesuai kebutuhan
                    });
                }).catch(error => {
                    // Tangani respons error (misal: validasi error)
                    let errorMessages = '';
                    if (error.errors) {
                        // Kumpulkan semua pesan error validasi
                        for (const key in error.errors) {
                            if (error.errors.hasOwnProperty(key)) {
                                errorMessages += `${error.errors[key].join('<br>')}<br>`;
                            }
                        }
                    } else {
                    ...
                    Swal.fire({
                        title: 'Gagal!',
                        html: errorMessages, // Gunakan html untuk menampilkan line breaks
                        icon: 'error'
                    });
                });
            }

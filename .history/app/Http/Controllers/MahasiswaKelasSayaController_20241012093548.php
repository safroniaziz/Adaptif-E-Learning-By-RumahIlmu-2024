...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function postTest(Kelas $kelas, Materi $materi){
        $topikId = Materi::select('topik_pembahasan_id')
            ->where('some_condition', 'value') // Anda bisa menyesuaikan dengan kondisi spesifik jika diperlukan
            ->first();

$cekTugasKelompok = PengumpulanTugasKelompokDetail::with(['pengumpulanTugasKelompok.tugasKelompok.materi'])
    ->where('mahasiswa_id', Auth::user()->id)
    ->whereHas('pengumpulanTugasKelompok.tugasKelompok.materi', function($query) use ($topikId) {
        $query->where('topik_pembahasan_id', $topikId->topik_pembahasan_id);
    })
    ->count();

...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function generateFuzzy(Kelas $kelas, Materi $materi){
        try {
            ...
            $topic_baru = Page::join('topics','topics.id','pages.topicId')
                                ->select('pages.id as pageId','topics.id as topicId','urutanTopic')
                                ->where('urutanTopic',$urutan)
                                ->where('criticalStatus',$critical)

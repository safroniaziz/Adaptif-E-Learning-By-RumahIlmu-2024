        ...
        try {
            ...
            $cekPostTest = NilaiKuisMateri::with(['kuisMateri'])
                            ...
                            ->whereHas('kuisMateri', function ($query) use ($materi) {
                                $query->where('materi_id', $materi->id);
                                $query->where('jenis_kuis', 'posttest');
                            })
                            ->first();
            $cekJumlahKomentar = DiskusiRespon::select('nilai')
                                ->where('mahasiswa_id',Auth::user()->id)
                                ->whereHas('diskusi.materi', function($query) use($materi) {
                                    $query->where('topik_pembahasan_id', function($subQuery) use($materi) {
                                        // Mengambil topik_pembahasan_id dari Materi
                                ...
                                ->count();
            $cekNilaiKomentar = DiskusiRespon::select(DB::raw('sum(nilai) as nilai'))
                                ->where('mahasiswa_id',Auth::user()->id)
                                ->whereHas('diskusi.materi', function($query) use($materi) {
                                    $query->where('topik_pembahasan_id', function($subQuery) use($materi) {
                                        // Mengambil topik_pembahasan_id dari Materi
                                ...
                                ->first();
            return $cekTugasIndividu->rata_rata;
            if ($cekTugasIndividu->rata_rata > 2.1 && $cekTugasIndividu->rata_rata <=3) {
                $skor_individu = 3;
                $kategori_individu = "Kreasi";
            }elseif ($cekTugasIndividu->rata_rata >= 1.3 && $cekTugasIndividu->rata_rata <=2.1) {
                $skor_individu = 2;
                $kategori_individu = "Evaluasi";
            }elseif ($cekTugasIndividu->rata_rata >= 0 && $cekTugasIndividu->rata_rata <=1.29) {
                $skor_individu = 1;
                $kategori_individu = "Analisis";
            }
            return $skor_individu;

...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function utsKelas(Kelas $kelas){
        ...
        $batasWaktuTerakhir = $waktuMulai->copy()->addMinute();
        // Logika pengecekan waktu masuk UTS
        if ($tanggalSekarang->lt($batasWaktuMasuk)) {
            // Jika saat ini sebelum 10 menit sebelum UTS dimulai
            return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                                ->with('error', 'UTS akan dilaksanakan pada tanggal ' . $tanggalUts->translatedFormat('l, d F Y') .
                        ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '. Anda hanya bisa masuk ke halaman UTS 10 menit sebelum dimulai.');
        } elseif ($tanggalSekarang->between($batasWaktuMasuk, $batasWaktuTerakhir)) {
            // Jika sekarang antara 10 menit sebelum UTS dimulai hingga 1 menit setelah dimulai
            $pesan = 'Anda bisa masuk ke halaman UTS sekarang.';
        } elseif ($tanggalSekarang->gt($batasWaktuTerakhir) && $tanggalSekarang->lt($waktuSelesai)) {
            // Jika sekarang antara 1 menit setelah UTS dimulai hingga waktu selesai
            $pesan = 'Anda masih bisa masuk ke halaman UTS, namun UTS telah dimulai.';
        } elseif ($tanggalSekarang->gt($waktuSelesai)) {
            // Jika UTS sudah selesai
            return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                            ->with('error', 'UTS sudah dilaksanakan pada hari ' . $tanggalUts->translatedFormat('l, d F Y') .
                                ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '.');
        } else {
            $pesan = null; // Jika tidak ada pesan
        }

...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function generateFuzzy(Kelas $kelas, Materi $materi){
        try {
            ...
            if (empty($isCreated) && empty($isEnrolled)) {
                RiwayatFuzzy::create([
                    'mahasiswa_id'         =>  Auth::user()->id,
                    'materi_id_sebelumnya'      =>  $topikLama->id,
                    'pageIdAfter'       =>  $topikBaru->id,
                    'score_post_test'   =>  $skorPostTest,
                    'score_individu'    =>  $skorIndividu,
                    'score_kelompok'    =>  $skorKelompok,
                    'score_diskusi'     =>  $skorDiskusi,
                    'skorberfikir_kritis'   =>  $bk,

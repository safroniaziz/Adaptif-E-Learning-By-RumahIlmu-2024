...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function postTest(Kelas $kelas, Materi $materi){
        $topikId = Materi::select('topik_pembahasan_id')->first();
        $cekTugasKelompok = PengumpulanTugasKelompokDetail::with(['pengumpulanTugasKelompok.tugasKelompok.materi'])->where('mahasisaw_id',Auth::user()->id)->count();

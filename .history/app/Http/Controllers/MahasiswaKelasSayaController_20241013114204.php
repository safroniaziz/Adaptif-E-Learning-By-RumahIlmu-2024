...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function utsKelasPost(Request $request, Kelas $kelas){
        $soals = UtsSoal::with(['uts.bankSoalPembahasans.jawabans',])
            ->whereHas('uts', function($query) use ($kelas) {
                $query->where('kelas_id', $kelas->id);
            })
            ->get();
        ...
        session()->flash('success', 'Jawaban uts berhasil disimpan!');
        return redirect()->route('mahasiswa.dashboard.kelas_saya',[$kelas->id]);
    }

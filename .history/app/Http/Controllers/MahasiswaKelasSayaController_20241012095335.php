...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function postTest(Kelas $kelas, Materi $materi){
        ...
        if ($cekDiskusi > 0) {
            ...
            $notification = array(
                'message' => 'Ooopps, anda tidak dapat mengerjakan post test, silahkan memberikan komentar pada forum diskusi minimal 1 kali terlebih dahulu!',
                'alert-type' => 'error'
            );
            return view('mahasiswa/kelas_saya/postTest');
            return redirect()->route('student.my_course.forum',[$pageId])->with($notification);
        }
    }
}

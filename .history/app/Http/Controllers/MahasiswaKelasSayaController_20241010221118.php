...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function forumDiskusi(Kelas $kelas, Materi $materi){
        $diskusis = Diskusi::with(['mahasiswa','diskusiRespons'])->withCount(['diskusiRespons'])->where('materi_id',$materi->id)->orderBy('created_at','desc')->get();
        return $diskusis->first()->diskusiRespons;

...
{
    try {
        $uts = Uts::where('kelas_id', $kelas->id)->first();

        if (!$uts) {
            throw new \Exception("UTS tidak ditemukan untuk kelas ini.");
        }

        $soals = $uts->bankSoalPembahasans()->with('jawabans')->get();

        if ($soals->isEmpty()) {
            throw new \Exception("Tidak ada soal yang ditemukan untuk UTS ini.");
        }
        ...
        foreach ($soals as $index => $soal) {
            $soal_id = $request->input("soal_{$index}");
            $jawaban_id = $request->input("jawaban_{$index}");
            $alasan = $request->input("alasan_{$index}");

            if (!$soal_id || !$jawaban_id) {
                $errors[] = "Data tidak lengkap untuk soal index {$index}";
                continue;
            }
            ...
            if (!$jawabanBenar) {
                $errors[] = "Jawaban benar tidak ditemukan untuk soal_id: {$soal_id}";
                continue;
            }
        ...
        if (empty($nilais)) {
            throw new \Exception("Tidak ada data nilai yang valid untuk disimpan. Errors: " . implode(", ", $errors));
        }
        ...
        \Log::error("Error in utsKelasPost: " . $e->getMessage());
        return response()->json([
            'success' => false,

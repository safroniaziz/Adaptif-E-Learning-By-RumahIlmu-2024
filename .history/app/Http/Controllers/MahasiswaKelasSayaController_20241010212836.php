...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function forumDiskusi(Kelas $kelas, Materi $materi){
        $diskusiSayas = Diskusi::with(['diskusiRespons'])->where('materi_id',$materi->id)->where('mahasiswa_id',auth()->user()->id)->get();
        $semuaDiskusis = Diskusi::with(['diskusiRespons'])->where('materi_id',$materi->id)->where('mahasiswa_id',auth()->user()->id)->get();

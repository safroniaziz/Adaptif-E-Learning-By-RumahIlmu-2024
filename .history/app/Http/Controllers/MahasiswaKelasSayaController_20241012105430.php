...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function postTest(Kelas $kelas, Materi $materi){
        ...
        if ($cekTugasKelompok > 0) {
            $alertTugasKelompok = false;
        }else {
            $alertTugasKelompok = true;
        }

        if ($cekTugasIndividu > 0) {
            $alertTugasIndividu = false;
        }else {
            $alertTugasIndividu = true;
        }

        $alertKuis = false;
        if ($alertDiskusi == false && $alertTugasIndividu == false && $alertTugasKelompok == false) {
            $kuisPostTest = KuisMateri::where('materi_id',$materi->id)->where('jenis_kuis','posttest')->first();
            if ($kuisPostTest) {
                $nilaiKuisPostTest = NilaiKuisMateri::where('mahasiswa_id',Auth::user()->id)->where('kuis_materi_id',$kuisPostTest->id)->first();

                if ($nilaiKuisPostTest) {
                    $isSelesai = true;
                }else {
                    $isSelesai = false;
                }

                $soalKuis = PertanyaanKuisMateri::with(['bankSoalPembahasan'])->where('kuis_materi_id',$kuisPostTest->id)->get();
                return $soalKuis;
            }else {
                $alertKuis = true;
            }
        }
        return $nilaiKuisPostTest;

        return view('mahasiswa/kelas_saya/post_test.index', compact('kelas','materi','alertDiskusi','alertTugasKelompok','alertTugasIndividu'));
    }
}

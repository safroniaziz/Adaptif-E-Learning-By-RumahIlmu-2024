...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function utsKelasPost(Request $request, Kelas $kelas){
        $soal = Uts::with(['bankSoalPembahasans.jawabans',])
            ->where('kelas_id', $kelas->id)
            ->first();
        $nilais = [];
        return $soal->bankSoalPembahasans;

        foreach ($soal->bankSoalPembahasans as $index => $soal) {
            ...
            $nilais[] = [
                ...
                'nilai' => $nilai,
                'alasan' => $alasan,
                'created_at' => now(), // Mengisi created_at
                'updated_at' => now(), // Mengisi updated_at
            ];
        }

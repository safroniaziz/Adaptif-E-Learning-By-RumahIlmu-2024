...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function generateFuzzy(Kelas $kelas, Materi $materi){
        try {
            ...
            if ($cekPostTest->nilai >= 8 && $cekPostTest->nilai <=10) {
                $skor_post = 3;
                $kategori_post = "Tinggi";
            }elseif ($cekPostTest->nilai >= 4 && $cekPostTest->nilai <=7) {
                $skor_post = 2;
                $kategori_post = "Sedang";
            }elseif ($cekPostTest->nilai >= 0 && $cekPostTest->nilai <=3) {
                $skor_post = 1;
                $kategori_post = "Rendah";
            }

            return $kategoriKelompok;

        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                return redirect()->back()
                    ...
                    ->withInput()
                    ->with('error', 'Mohon jawab semua pertanyaan sebelum mengirim.');
            }
            ...
            foreach ($kuis as $index => $pertanyaanKuis) {
                ...
                if (!$jawabanBenar) {
                    Log::error("Jawaban benar tidak ditemukan untuk kuis_id: {$kuis_id}");
                    throw new \Exception("Terjadi kesalahan dalam memproses jawaban.");
                }
            ...
            return redirect()
                ->route('mahasiswa.kelas_saya.post_test', [$kelas->id, $materi->id])
                ->with('success', 'Jawaban post test berhasil disimpan!');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Error in simpanKuisPosttest: ' . $e->getMessage());
            return redirect()->back()

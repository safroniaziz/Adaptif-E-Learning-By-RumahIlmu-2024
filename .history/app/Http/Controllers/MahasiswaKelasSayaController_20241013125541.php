...
{
    return $request->soal_1;
    // Mengambil data UTS dengan relasi soal dan jawaban
    $soalUts = Uts::with(['bankSoalPembahasans.jawabans'])
        ->where('kelas_id', $kelas->id)
        ->first();

    // Pastikan UTS ditemukan
    if (!$soalUts) {
        return response()->json([
            'message' => 'Soal UTS tidak ditemukan.',
        ], 404);
    }

    $nilais = []; // Array untuk menyimpan hasil penilaian

    foreach ($soalUts->bankSoalPembahasans as $index => $soalPembahasan) {
        $soal_id = $request->input("soal_{$index}"); // Mengambil input soal dari request
        $jawaban_id = $request->input("jawaban_{$index}"); // Mengambil jawaban dari request
        $alasan = $request->input("alasan_{$index}"); // Mengambil alasan dari request

        // Mencari jawaban yang benar
        $jawabanBenar = JawabanKuisMateri::where('bank_soal_pembahasan_id', $soal_id)
            ->where('status_jawaban', true)
            ->first();

        // Jika jawaban benar tidak ditemukan, kembalikan pesan error
        if (!$jawabanBenar) {
            return response()->json([
                'message' => 'Jawaban benar tidak ditemukan untuk soal ID: ' . $soal_id,
            ], 404);
        }

        // Cek apakah jawaban yang dipilih sama dengan jawaban yang benar
        $nilai = ($jawaban_id == $jawabanBenar->id) ? 1 : 0;

        // Tambahkan hasil penilaian ke array $nilais
        $nilais[] = [
            'uts_id' => $soalUts->id,
            'mahasiswa_id' => Auth::user()->id,
            'bank_soal_pembahasan_id' => $soal_id,
    ...
    // Mengembalikan respons JSON jika requestnya adalah AJAX atau JSON
    ...
    // Jika bukan AJAX, redirect ke halaman kelas
    return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
        ->with('message', 'Jawaban UTS berhasil disimpan!');
}

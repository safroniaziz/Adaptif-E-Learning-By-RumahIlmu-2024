...
class MahasiswaKelasSayaController extends Controller
{
    ...
    ->where('mahasiswa_id', Auth::user()->id)
    ->whereHas('pengumpulanTugasKelompok.tugasKelompok.materi', function($query) {
        $query->where('topik_pembahasan_id', function($subQuery) {
            // Mengambil topik_pembahasan_id dari Materi
            $subQuery->select('topik_pembahasan_id')
                     ->from('materis')
                     ->orderBy('id') // Dapat menyesuaikan urutan jika diperlukan
                     ->limit(1); // Ambil satu record
        });
    })
    ->count();

        ...
        try {
            ...
            foreach ($soals as $pertanyaan) {
                $skor_detail[] = array(
                    ...
                    'nilai' =>  $_POST['nilai_'.$pertanyaan->bankPenilaianKelompok->id],
                );
            }

            $total_skor =  array_sum(array_column($skor_detail, 'nilai'));
            $rata_rata = $total_skor/$request->jumlah_pertanyaan;

            $penilaianKelompok = PenilaianKelompok::create([
                'topik_pembahasan_kelas_id'  => $materi->topik_pembahasan_kelas_id, // Sesuaikan dengan nama yang ada di migration
                'penilai_id'                 => Auth::user()->id, // Penilai adalah user yang sedang login
                'mahasiswa_id'               => $request->userId, // Id mahasiswa yang dinilai
                'pengalaman'                 => $request->pengalaman, // Sesuaikan dengan nama field di migration
                'kelompok'                   => $request->kelompok, // Kelompok diambil dari request
                'total_skor'                 => $total_skor, // Total skor hasil kalkulasi
                'rata_rata'                  => $rata_rata, // Nilai rata-rata hasil kalkulasi
            ]);


            for ($i=0; $i <count($skor_detail); $i++) {
                $detail[] = array(
                    'assesmentSheetId'  =>  $assesment->id,
                    'assesmentQuestId'  =>  $skor_detail[$i]['assesmentQuestId'],
                    'score'              =>  $skor_detail[$i]['skor'],
                );
            }

            return response()->json([
                'success' => true,
                'skor_detail' => $skor_detail, // Kirim nilai $average ke frontend
                'total_skor' => $total_skor, // Kirim nilai $average ke frontend
                'rata_rata' => $rata_rata, // Kirim nilai $average ke frontend
            ...
            return response()->json([
                'error' => 'Terjadi kesalahan saat menyimpan penilaian.',
            ], 500);
        }
    }

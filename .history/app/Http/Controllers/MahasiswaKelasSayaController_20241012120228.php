...
class MahasiswaKelasSayaController extends Controller
{
   ...
   {
       Log::info('Memulai simpanKuisPosttest', [
           'user_id' => Auth::id(),
           'kelas_id' => $kelas->id,
           'materi_id' => $materi->id
       ]);
       ...
       try {
           ...
           Log::info('Post test ditemukan', ['post_test_id' => $postTest->id]);

           $kuis = PertanyaanKuisMateri::with('bankSoalPembahasan.jawabans')
               ->where('kuis_materi_id', $postTest->id)
               ->get();

           Log::info('Jumlah pertanyaan kuis', ['count' => $kuis->count()]);
           ...
           Log::info('Kuis post test berhasil disimpan', [
               'user_id' => Auth::id(),
               'kelas_id' => $kelas->id,
               'materi_id' => $materi->id
           ]);
           ...
           return response()->json([
               'success' => true,
               'message' => 'Jawaban post test berhasil disimpan!',
               'redirect' => route('mahasiswa.kelas_saya.post_test', [$kelas->id, $materi->id])
           ]);

       } catch (\Exception $e) {
           DB::rollBack();
           Log::error('Error dalam simpanKuisPosttest', [
               'error' => $e->getMessage(),
               'user_id' => Auth::id(),
               'kelas_id' => $kelas->id,
               'materi_id' => $materi->id

...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function detailKelas(Kelas $kelas){
        ...
        // $statusUts = UtsPeserta::with(['utsSesi.uts' => function($query) use ($kelas) {
        ...
        // if (!$statusUts) {
        //     // Handle jika mahasiswa tidak terdaftar di sesi UTS
        //     $pesan = 'Anda tidak terdaftar dalam sesi UTS manapun.';
        //     return $pesan;
        // }

        // $tanggalSekarang = Carbon::now();
        // $tanggalUts = Carbon::parse($statusUts->utsSesi->tanggal_dilaksanakan);

        // // Parsing waktu dari format AM/PM ke Carbon
        // $waktuMulai = Carbon::createFromFormat('h:i A', $statusUts->utsSesi->waktu_mulai);
        // $waktuSelesai = Carbon::createFromFormat('h:i A', $statusUts->utsSesi->waktu_selesai);

        // // Logika pengecekan tanggal UTS
        // if ($tanggalUts->isFuture()) {
        //     $pesan = 'UTS akan dilaksanakan pada tanggal ' . $tanggalUts->translatedFormat('l, d F Y') .
        //              ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '.';
        // } elseif ($tanggalUts->isPast()) {
        //     $pesan = 'UTS sudah dilaksanakan pada tanggal ' . $tanggalUts->translatedFormat('l, d F Y') .
        //              ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '.';
        // } else {
        //     $pesan = null; // Jika tidak ingin ada pesan
        // }
        return view('mahasiswa/kelas_saya.detail_kelas',compact('kelas'));
    }

    public function utsKelas(Kelas $kelas){
        ...
        // Mendapatkan tanggal UTS dari data peserta
        $tanggalUts = Carbon::parse($peserta->utsSesi->tanggal_dilaksanakan);
        ...
        // Menghitung batas waktu masuk: 10 menit sebelum UTS dimulai
        $batasWaktuMasuk = $waktuMulai->copy()->subMinutes(10);

        // Menghitung batas waktu terakhir masuk: 2 menit setelah UTS dimulai
        $batasWaktuTerakhir = $waktuMulai->copy()->addMinutes(2);

        // Kombinasikan tanggal UTS dengan waktu mulai dan selesai
        $waktuMulaiLengkap = $tanggalUts->copy()->setTimeFrom($waktuMulai);
        $waktuSelesaiLengkap = $tanggalUts->copy()->setTimeFrom($waktuSelesai);

        // Fungsi untuk membuat pesan dengan format yang diminta
        function buatPesan($tanggalUts, $waktuMulai, $waktuSelesai) {
            return 'UTS akan dilaksanakan pada tanggal ' . $tanggalUts->translatedFormat('l, d F Y') .
                ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '.';
        }

        // Logika pengecekan waktu UTS
        if ($tanggalSekarang->lt($tanggalUts)) {
            // Jika tanggal sekarang kurang dari tanggal UTS
            return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                ->with('error', buatPesan($tanggalUts, $waktuMulai, $waktuSelesai) .
                    ' Anda hanya bisa masuk ke halaman UTS 10 menit sebelum dimulai.');
        } elseif ($tanggalSekarang->gt($tanggalUts)) {
            // Jika tanggal sekarang lebih dari tanggal UTS
            return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                ->with('error', 'UTS sudah dilaksanakan pada hari ' . $tanggalUts->translatedFormat('l, d F Y') .
                    ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '.');
        } else {
            // Jika tanggal sekarang sama dengan tanggal UTS
            if ($tanggalSekarang->lt($batasWaktuMasuk)) {
                // Jika masih lebih dari 10 menit sebelum waktu mulai UTS
                return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                    ->with('error', buatPesan($tanggalUts, $waktuMulai, $waktuSelesai) .
                        ' Anda hanya bisa masuk ke halaman UTS 10 menit sebelum dimulai.');
            } elseif ($tanggalSekarang->between($batasWaktuMasuk, $waktuMulaiLengkap)) {
                // Jika waktu sekarang antara 10 menit sebelum UTS dimulai dan waktu mulai UTS
                return view('mahasiswa/kelas_saya/ujian.uts', compact('kelas', 'peserta'));
            } elseif ($tanggalSekarang->between($waktuMulaiLengkap, $batasWaktuTerakhir)) {
                // Jika waktu sekarang antara waktu mulai UTS dan 2 menit setelahnya
                return view('mahasiswa/kelas_saya/ujian.uts', compact('kelas', 'peserta'));
            } else {
                // Jika waktu sekarang lebih dari 2 menit setelah waktu mulai UTS
                return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                    ->with('error', 'UTS sudah dilaksanakan pada hari ' . $tanggalUts->translatedFormat('l, d F Y') .
                        ' dari pukul ' . $waktuMulai->format('H:i') . ' sampai ' . $waktuSelesai->format('H:i') . '.');
            }

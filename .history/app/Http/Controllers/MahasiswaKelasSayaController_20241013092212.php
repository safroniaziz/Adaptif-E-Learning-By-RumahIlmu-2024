...
class MahasiswaKelasSayaController extends Controller
{
    ...
    public function detailKelas(Kelas $kelas){
        ...
        $statusUts = UtsPeserta::with(['utsSesi.uts' => function($query) use ($kelas) {
                    ...
                    ->first();
        if ($statusUts->utsSesi->tanggal_dilaksanakan > tanggal sekarang) {
            return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                 ->with('error', 'Uts akan dilaksanakan pada tanggal $statusUts->utsSesi->tanggal_dilaksanakan.');
        }elseif ($statusUts->utsSesi->tanggal_dilaksanakan > tanggal sekarang) {
        }{
            return redirect()->route('mahasiswa.kelas_saya.detail_kelas', [$kelas->id])
                 ->with('error', 'Uts sudah dilaksanakan pada tanggal $statusUts->utsSesi->tanggal_dilaksanakan.');
        }

...
@section('subTitle','Bank Kuisioner')
@section('page','Bank Kuisioner')
@section('login_as')
    Selamat Datang,
@endsection
@section('user-login2')
    ...
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Manajemen Data Bank Kuisioner</h3>
            </div>
            <div class="box-body">
                <div class="row">
                    <form action="{{ route('bankKuisioner.post') }}" method="POST" enctype="multipart/form-data" class="form">
                        {{ csrf_field() }} {{ method_field("POST") }}
                        <div class="form-group col-md-12">
                            <label for="">Pertanyaan Kuisioner</label>
                            <input type="text" value="{{ old('pertanyaan') }}" name="pertanyaan" id="pertanyaan" class="form-control">
                            <div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="jenis_kuisioner_id" id="jenis_kuisioner_id" class="form-control @error('jenis_kuisioner_id') is-invalid @enderror">
                                ...
                                @foreach ($jenisKuisioners as $jenisKuisioner)
                                    <option value="{{ $jenisKuisioner->id }}">{{ $jenisKuisioner->jenis_kuisioner }}</option>
                                @endforeach
                            </select>
                            <div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="tipe_kuisioner" class="form-control" id="tipe_kuisioner">
                                ...
                                <option value="positif">Pertanyaan Positif</option>
                                <option value="negatif">Pertanyaan Negatif</option>
                            </select>
                            <div>
                        ...
                        <div class="col-md-12" style="text-align: center">
                            ...
                            <button type="submit" class="btn btn-primary btn-sm btn-flat btnSubmit"><i class="fa fa-check-circle"></i>&nbsp; Simpan Kuisioner</button>
                        ...
                        <table class="table table-striped table-bordered" id="table" style="width:100%;">
                            ...
                            <tbody>
                                @php
                                    $no=1;
                                @endphp
                                @foreach ($bankKuisioners as $bankKuisioner)
                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        <td>{{ ucfirst($bankKuisioner->pertanyaan) }}</td>
                                        <td>{{ ucfirst($bankKuisioner->jenisKuisioner->jenis_kuisioner) }}</td>
                                        <td>{{ ucfirst($bankKuisioner->tipe_kuisioner) }}</td>

                                        <td style="display:inline-block !important;">
                                            <table>
                                                <tr>
                                                    <td>
                                                        <a href="{{ route('bankKuisioner.edit',[$bankKuisioner->id]) }}" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-edit"></i>&nbsp; Edit</a>
                                                    </td>
                                                    <td>
                                                        <form action="{{ route('bankKuisioner.delete',[$bankKuisioner->id]) }}" method="POST">
